...
function MediaPageComponent(props) {
  ...
  const { isLoading: PastEventsIsLoading, sendRequest: sendPastEventsRequest } =
    useHttpClient();
    ...
    async (EntryPerPage, page_number) => {
      try {
        const responseData = await sendPastEventsRequest(
          ...
          }/events?_limit=${EntryPerPage}&_start=${
            EntryPerPage * (page_number - 1)
          }&_sort=Event_date:DESC`
        );
    ...
    return (
      <>
        ...
        <PaginationComponent
          ...
          LoadedItemsCount={LoadedEventsCount}
        />
      </>
    );
  }
