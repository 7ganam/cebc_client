...
function NewsEventSubPagesComponent(props) {
  ...
  const fetch_News_count = useCallback(async () => {
    try {
      const responseData = await sendNewsRequest(
        `${process.env.REACT_APP_BACKEND_URL}/news/count`
      );
  ...
  const {
    ...
    sendRequest: sendPastEventsRequest,
    clearError: clearPastError,
  } = useHttpClient();
    ...
    async (EntryPerPage, page_number) => {
      try {
        ...
        const responseData = await sendPastEventsRequest(
          ...
          }/events?Event_date_lt=${current_date}&_limit=${EntryPerPage}&_start=${
            EntryPerPage * (page_number - 1)
          }`
        );
  ...
  const fetch_News = useCallback(async (EntryPerPage, page_number) => {
    try {
      const responseData = await sendNewsRequest(
        `${
          process.env.REACT_APP_BACKEND_URL
        }/news?_sort=date:DESC&_limit=${EntryPerPage}&_start=${
          EntryPerPage * (page_number - 1)
        }`
      );
  ...
  const render_upcomming_events = (upcomming_events) => {
    ...
    for (const event of upcomming_events) {
      ...
      if (!events_object[year]) {
        events_object[year] = {};
      }
      if (!events_object[year][month]) {
        events_object[year][month] = [];
      }
  ...
  const render_past_events = (past_events) => {
    ...
    for (const event of past_events) {
      ...
      events_object[year][month].push(event);
      events_object[year][month].sort((a, b) =>
        moment(a.Event_date) > moment(b.Event_date) ? 1 : -1
      );
    }
              ...
              <Card className="past_event_card mb-4">
                {event.Event_thumbnail_image ? (
                  <CardImg
                    ...
                    src={s3_map(`${event.Event_thumbnail_image.url}`)}
                    alt="Card image cap"
                  />
                ) : (
                  <CardImg
    ...
    const ordered_years = Object.keys(events_object)
      .sort()
      .reverse((obj, key) => {
      ...
      render_event.push(
        <>
          ...
          <Container style={{ marginTop: "50px" }}>
            <Row>{reander_sub_elements(events_object, year)}</Row>
          </Container>
        </>
      );
    }
  ...
  const render_news = (past_newss) => {
    ...
    for (const news of past_newss) {
      ...
      if (!newss_object[year]) {
        newss_object[year] = {};
      }
      if (!newss_object[year][month]) {
        newss_object[year][month] = [];
      }

      newss_object[year][month].unshift(news);
      newss_object[year][month].sort((a, b) =>
              ...
              <Card className="past_news_card mb-4">
                {news.thumbnail_image ? (
                  <CardImg
                    ...
                    width="100%"
                    style={{ height: "100px", objectFit: "contain" }}
                    src={s3_map(`${news.thumbnail_image.url}`)}
                    alt="Card image cap"
                  />
                ) : (
                  <CardImg
    ...
    const ordered_years = Object.keys(newss_object)
      .sort()
      .reverse((obj, key) => {
    ...
    for (const year of ordered_years) {
      render_news.push(
        <>
          ...
          <Container style={{ marginTop: "50px" }}>
            <Row>{reander_sub_elements(newss_object, year)}</Row>
          </Container>
        </>
      );
    }
    ...
    if (sub_par === "All") {
      return (
        <>
          ...
          {upcomming.length > 0 || past.length > 0 ? (
            <>
              {upcomming.length > 0 && render_upcomming_events(upcomming)}
              {!PastEventsIsLoading ? (
                past.length > 0 && (
                  <div
                    style={{
          ...
          <PaginationComponent
            fetch_items={fetch_past_Events}
            items_per_page={events_per_page}
            LoadedItemsCount={PastEventsCount}
          />
        </>
      );
          ...
          <Container style={{ marginTop: "200px" }}>
            <Row className=" justify-content-center">
              <div class="section_header" style={{ marginTop: "30px" }}>
                <span class="section_header_inner">Past events</span>
                <div className="section_header_under"></div>
              </div>
            </Row>
          </Container>
      ...
      return (
        <>
          <Container style={{ marginTop: "200px" }}>
            <Row className=" justify-content-center">
              <div class="section_header" style={{ marginTop: "30px" }}>
                <span class="section_header_inner">Upcomming events</span>
                <div className="section_header_under"></div>
              </div>
            </Row>
          </Container>
      ...
      return (
        <>
          ...
          <PaginationComponent
            fetch_items={fetch_News}
            items_per_page={news_per_page}
            LoadedItemsCount={LoadedNewsCount}
          />
        </>
      );
    }
  ...
  return (
    <div style={{ marginBottom: "20px" }}>
      {generate_page_view(
        props.match.params.sub_parameter,
        LoadedUpcommingEvents,
        LoadedPastEvents
      )}
    </div>
  );
