  ...
  function generate_case_studies_views(case_studies) {
    const members_views = case_studies.map((case_study, index) => {
      return (
        <Link
          className="report_link"
          to={`/KNOWLEDGECENTER/CASE_STUDIES/${case_study.slug}`}
        >
          <div className="report_container">
            <i class="fas fa-external-link-alt mr-2 ml-1"></i>
            {` ${case_study.title}`}
  ...
  function generate_news_views(news) {
    const members_views = news.map((news_post, index) => {
      return (
        <Link
          className="report_link"
          to={`/EVENTS_NEWS/NEWS/${news_post.slug}`}
        >
          <div className="report_container">
            <i class="fas fa-external-link-alt mr-2 ml-1"></i>
            {` ${news_post.title}`}
                ...
                <div className="report_image_box">
                  {member.entity_image ? (
                    <img
                      ...
                      src={s3_map(
                        `${member.entity_image && member.entity_image.url}`
                      )}
            ...
            <Col className="p-0" md={7}>
                    ...
                    <div id="member_body">
                      <div style={{}}>
                        ...
                        {member.entity_website && (
                          <div
                            className="open_report_button"
                            style={{ marginTop: "20px" }}
                          >
                            <a
                              target="_blank"
                              href={member.entity_website}
                              style={{ textDecoration: "none" }}
                              rel="noreferrer"
                            >
                              <div
                                className="report_publication_button"
                ...
                <>
                  <Row className="title_row">
                    <div className="section_header section_header_side">
                      <span className="section_header_inner">Projects</span>
                      <div className="section_header_under"></div>
                    </div>
                  </Row>

                  <Row>{generate_projects_cards(member.projects)}</Row>
                </>
              )}
              {!!member && !!member.users && !!member.users.length > 0 && (
                <Container>
                  ...
                  <Row>{generate_members_view(member.users)}</Row>
                </Container>
              )}
              {!!member &&
                !!member.reports_publications &&
                !!member.reports_publications.length > 0 && (
                  <Container>
                    <Row className="title_row">
                      <div className="section_header section_header_side">
              ...
              {!!member && !!member.news && !!member.news.length > 0 && (
                <>
                  <Container>
                    <Row className="title_row">
                      <div className="section_header section_header_side">
                        <span className="section_header_inner">News</span>
                        <div className="section_header_under"></div>
                      </div>
                    </Row>
                    <Row>{generate_news_views(member.news)}</Row>
                  </Container>
                </>
              )}
            </Col>
