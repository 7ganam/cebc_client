...
function PresintationsPageComponent() {
  const {
    isLoading: PresentationsIsLoading,
    error: PresentationsError,
    sendRequest: sendPresentationsRequest,
    clearError,
  } = useHttpClient();
  const [LoadedPresentations, setLoadedPresentations] = useState([]);
  const fetch_Presentations = useCallback(async () => {
    try {
  ...
  function generate_presentations_view() {
    ...
    for (const presentation_group of LoadedPresentations) {
      if (
        ...
        moment(presentation_group.date).year() !== latest_date
      ) {
        latest_date = moment(presentation_group.date).year();
        presentation_container.push(
          <div
            className="presentation_group_title"
            style={{
              textAlign: "center",
              fontSize: "35px",
              borderBottom: "3px solid #bebebe",
              marginBottom: "50px",
              marginTop: "70px",
        ...
        presentation_container.push(
          <div className="presentation_box">
            <a
              ...
              target="_blank"
              href={s3_map(presentation.file && presentation.file.url)}
              rel="noreferrer"
            >
