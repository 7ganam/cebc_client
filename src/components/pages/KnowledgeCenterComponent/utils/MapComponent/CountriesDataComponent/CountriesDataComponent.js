...
function CountriesDataComponent(props) {
    ...
    let projects_view = country.projects.map((project, index) => {
      return (
        <div className="data_box">
          <Link
            className="data_link"
            to={`/KNOWLEDGECENTER/PROJECTS/${project.slug}`}
          >
            <i class="fas fa-external-link-alt mr-2 ml-1"></i>
  ...
  function generate_selected_countries_views(
    selected_countries,
    loaded_counties
  ) {
    let views = selected_countries.map((country, country_index) => {
      let loaded_country = loaded_counties.filter(
        (loaded_country) => country === loaded_country.alpha2_code
      );
      loaded_country = loaded_country[0];
      return (
        <li
          key={loaded_country.id}
          className={`country_data_item
                    index_${country_index === 0 ? "first" : country_index} 
                    index_${
                      country_index === selected_countries.length - 1
                        ? "last"
                        : country_index
                    }
                    `}
        >
          <div
            className={`country_data_header 
                            index_${
                              country_index === 0 ? "first" : country_index
                            } 
                            index_${
                              country_index === selected_countries.length - 1
                                ? "last"
          ...
          <UncontrolledCollapse
            className="country_data_body"
            toggler={`#toggler_${loaded_country.id}`}
          >
  ...
  return (
    <div>
      {!!props.countries && props.Loadedcountrys && (
        <ul style={{ width: "100%" }}>
          {generate_selected_countries_views(
            props.countries,
            props.Loadedcountrys
          )}
        </ul>
      )}
    </div>
