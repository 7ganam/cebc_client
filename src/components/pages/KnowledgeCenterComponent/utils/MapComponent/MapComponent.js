...
function MapComponent() {
  const {
    isLoading: countrysIsLoading,
    error: countrysError,
    sendRequest: sendcountrysRequest,
    clearError,
  } = useHttpClient();
  const [Loadedcountrys, setLoadedcountrys] = useState([]);
  const fetch_countrys = useCallback(async () => {
    try {
      const responseData = await sendcountrysRequest(
  ...
  function hide_countries(shown_country_list) {
    ...
    if (russia.length > 0) {
      for (let country_to_hide of countries_to_hide) {
        let country_node = document.querySelectorAll(
          `[data-code=${country_to_hide.alpha2}`
        );
      ...
      setTimeout(() => {
        hide_countries(shown_country_list);
      }, 200);
    }
  }
