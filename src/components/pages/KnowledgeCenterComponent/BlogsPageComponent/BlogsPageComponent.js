...
function BlogsPageComponent(props) {
  const render_blogs = (blogs) => {
    ...
    for (const blog of blogs) {
      ...
      if (!blogs_object[year]) {
        blogs_object[year] = {};
      }
      if (!blogs_object[year][month]) {
        blogs_object[year][month] = [];
      }
    ...
    render_blog.push(
      <div class="section_header" style={{ marginTop: "100px" }}>
        <span class="section_header_inner">blogs</span>
        <div className="section_header_under"></div>
      </div>
    );

    const ordered_years = Object.keys(blogs_object)
      .sort()
      .reverse((obj, key) => {
      ...
      render_blog.push(
        <>
          ...
          <Container style={{ marginTop: "50px" }}>
            <Row>{reander_sub_elements(blogs_object, year)}</Row>
          </Container>
        </>
      );
    }
  ...
  return (
    <div style={{ marginTop: "200px", marginBottom: "50px" }}>
      {props.blogs.length > 0 && <div>{render_blogs(props.blogs)}</div>}
    </div>
  );
}
