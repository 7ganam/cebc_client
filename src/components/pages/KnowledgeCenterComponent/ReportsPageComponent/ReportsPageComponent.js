...
function ReportsPageComponent(props) {
  const render_reports = (reports) => {
    ...
    for (const report of reports) {
      ...
      if (!reports_object[year]) {
        reports_object[year] = {};
      }
      if (!reports_object[year][month]) {
        reports_object[year][month] = [];
      }

      reports_object[year][month].unshift(report);
      reports_object[year][month].sort((a, b) =>
              ...
              <Card className="past_report_card">
                {report.image ? (
                  <CardImg
                    ...
                    src={s3_map(`${report.image && report.image.url}`)}
                    alt="Card image cap"
                  />
                ) : (
                  <CardImg
    ...
    const ordered_years = Object.keys(reports_object)
      .sort()
      .reverse((obj, key) => {
      ...
      render_report.push(
        <>
          ...
          <Container style={{ marginTop: "50px" }}>
            <Row>{reander_sub_elements(reports_object, year)}</Row>
          </Container>
        </>
      );
    }
  ...
  return (
    <div style={{ marginTop: "0px", marginBottom: "50px" }}>
      ...
      {props.reports.length > 0 && <div>{render_reports(props.reports)}</div>}
    </div>
  );
}
