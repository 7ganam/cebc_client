...
function useInitLoadedData() {
  const {
    isLoading: ProgrammesIsLoading,
    error: ProgrammesError,
    sendRequest: sendProgrammesRequest,
    clearError,
  } = useHttpClient();
  const [LoadedProgrammes, setLoadedProgrammes] = useState([]);
  const fetch_Programmes = useCallback(async () => {
    try {
  ...
  const {
    ...
    error: ProjectsError,
    sendRequest: sendProjectsRequest,
  ...
  const fetch_News = useCallback(async () => {
    try {
      const responseData = await sendNewsRequest(
        `${process.env.REACT_APP_BACKEND_URL}/news?_limit=4&_sort=date:DESC`
      );
  ...
  const fetch_Events = useCallback(async () => {
    try {
      const responseData = await sendEventsRequest(
        `${process.env.REACT_APP_BACKEND_URL}/events?_limit=4&_sort=Event_date:DESC`
      );
  ...
  const {
    ...
    error: ReportsError,
    sendRequest: sendReportsRequest,
  ...
  const {
    isLoading: Case_studiesIsLoading,
    error: Case_studiesError,
    sendRequest: sendCase_studiesRequest,
    clearError: clearCase_studiesError,
  } = useHttpClient();
  const [LoadedCase_studies, setLoadedCase_studies] = useState([]);
  const fetch_Case_studies = useCallback(async () => {
    try {
  ...
  const {
    ...
    error: PodcastsError,
    sendRequest: sendPodcastsRequest,
    clearError: clearPodcastsError,
  } = useHttpClient();
  const [LoadedPodcasts, setLoadedPodcasts] = useState([]);
  const fetch_Podcasts = useCallback(async () => {
    try {
  ...
  const {
    ...
    error: WebinarsError,
    sendRequest: sendWebinarsRequest,
    clearError: clearWebinarsError,
  } = useHttpClient();
  const [LoadedWebinars, setLoadedWebinars] = useState([]);
  const fetch_Webinars = useCallback(async () => {
    try {
  ...
  const {
    ...
    error: BlogsError,
    sendRequest: sendBlogsRequest,
    clearError: clearBlogsError,
  } = useHttpClient();
  ...
  const {
    isLoading: PapersIsLoading,
    error: PapersError,
    sendRequest: sendPapersRequest,
    clearError: clearPapersError,
  } = useHttpClient();
  const [LoadedPapers, setLoadedPapers] = useState([]);
  const fetch_Papers = useCallback(async () => {
    try {
  ...
  const {
    isLoading: Entity_sIsLoading,
    error: Entity_sError,
    sendRequest: sendEntity_sRequest,
    clearError: clearEntity_sError,
  } = useHttpClient();
  const [LoadedEntity_s, setLoadedEntity_s] = useState([]);
  const fetch_Entity_s = useCallback(async () => {
    try {
  ...
  const {
    isLoading: Staff_membersIsLoading,
    error: Staff_membersError,
    sendRequest: sendStaff_membersRequest,
    clearError: clearStaff_membersError,
  } = useHttpClient();
  const [LoadedStaff_members, setLoadedStaff_members] = useState([]);
  const fetch_Staff_members = useCallback(async () => {
    try {
  ...
  const {
    ...
    error: CoursesError,
    sendRequest: sendCoursesRequest,
    clearError: clearCoursesError,
  } = useHttpClient();
  ...
  const {
    isLoading: NewslettersIsLoading,
    error: NewslettersError,
    sendRequest: sendNewslettersRequest,
    clearError: clearNewslettersError,
  } = useHttpClient();
  const [LoadedNewsletters, setLoadedNewsletters] = useState([]);
  const fetch_Newsletters = useCallback(async () => {
    try {
      const responseData = await sendNewslettersRequest(
        `${process.env.REACT_APP_BACKEND_URL}/newsletters?_limit=-1`
      );
